...
def get_more_probability(index):
    scenario = {
        'Оборудование': xw.Range(f'B{index-6}').value,
        'Сценарий': xw.Range(f'D{index-6}').value,
        'Номер сценария': xw.Range(f'A{index-6}').value,
...
#     определяем наиболее опасный и наиболее вероятный в блоке сценарий
    if xw.Range(f'L{index}').value not in components:
        components[xw.Range(f'L{index}').value] = [get_more_danger(index), get_more_probability(index)]
    else:
        dead_man = get_more_danger(index)['Погибшие']
        probability = get_more_probability(index)['Вероятность']

        if dead_man > components[xw.Range(f'L{index}').value][0]['Погибшие']:
            components[xw.Range(f'L{index}').value][0] = get_more_danger(index)

        if dead_man > components[xw.Range(f'L{index}').value][1]['Вероятность']:
            components[xw.Range(f'L{index}').value][1] = get_more_probability(index)
