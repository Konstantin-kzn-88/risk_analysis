...
def get_sum_risk(index):
    sum_risk = {
        'Коллективный риск гибели': sum(
            cell.value for cell in ws.range(f'AW{index - 6}:AW{index + 2}') if cell.value is not None),
        'Коллективный риск ранения': sum(
            cell.value for cell in ws.range(f'AX{index - 6}:AX{index + 2}') if cell.value is not None),
        'Индивидуальный риск гибели': sum(
            cell.value for cell in ws.range(f'AZ{index - 6}:AZ{index + 2}') if cell.value is not None),
        'Индивидуальный риск ранения': sum(
            cell.value for cell in ws.range(f'BA{index - 6}:BA{index + 2}') if cell.value is not None),
        # Fixed the Cyrillic 'ВА' to Latin 'BA'
        'Максимальный суммарный ущерб': max(
            cell.value for cell in ws.range(f'AV{index - 6}:AV{index + 2}') if cell.value is not None),
        'Максимальный экологический ущерб': max(
            cell.value for cell in ws.range(f'AU{index - 6}:AU{index + 2}') if cell.value is not None),
    }
    return sum_risk


# pp.pprint(get_sum_risk(8))
#
components = {}
#
...
for index in all_index_of_component:
    ...
    if xw.Range(f'L{index}').value not in components:
        components[xw.Range(f'L{index}').value] = get_sum_risk(index)
    else:
        risk = get_sum_risk(index)
